...
app.get("/sub/:token", (req, res) => {
    try {
        ...
        if (!user.active || new Date() > new Date(user.expires_at)) return res.status(403).send("Subscription expired")
...
app.get("/vpn", (req, res) => {
    try {
        ...
        console.error(err)
        return res.status(500).send("VPN file error")
    }
})
